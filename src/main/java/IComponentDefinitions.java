...
public class IComponentDefinitions {
	...
	protected String[] componentOptionsArray;
	...
	final void decode(BufferedStream stream) {
		...
		if ((type & 0x80 ^ 0xffffffff) != -1) {
			...
			//System.out.println("4765: " + aString4765);
		}
		anInt4814 = stream.readUnsignedShort();
		positionX = stream.readShort();
		...
		if (type == 4) {
			...
			//System.out.println("4790: " + aString4790);
			if (aString4790.toLowerCase().contains("runescape")) {
				aString4790 = aString4790.replace("RuneScape", ClientConstants.NAME).replace("runescape", ClientConstants.NAME).replace("Runescape", ClientConstants.NAME);
			}
		...
		//System.out.println("4779: " + aString4779);
		int i_25_ = stream.readUnsignedByte();
		int i26 = 0xf & i_25_; //let me just refractor it to look like mine
		int interfaceId = parentId >> 16;
		
		if (i26 == 0 && positionX == 22 && width == (2 * 150) + 29) {
			System.out.println("No Options: " + i_25_ + " : " + interfaceId);
			i26 = 1;
			i_25_ = 1;
			i_20_ = 2;
			componentOptionsArray = new String[1];
			componentOptionsArray[0] = "";
			for (int k = 0; k < aByteArray422.length; k++) {
				componentOptionsArray[0] += (char) aByteArray422[k];
			}
		} else if (i26 > 0) {
			componentOptionsArray = new String[i26];
			for (int idx = 0; idx < i26; idx++) {
				if (skillCount < 0) {
					skillCount = 24;
				}
				componentOptionsArray[idx] = stream.readString();
//				if(componentOptionsArray[idx].equals("Delete"))
//					componentOptionsArray[idx] = "Next Page";
				if (componentOptionsArray[idx].toLowerCase().contains("task system")) {
					componentOptionsArray[idx] = "Information Tab";
				}
			}
		}
//		if (interfaceId == 156) {
//			System.out.println("idx=" + i26);
//			for (int idx = 0; idx < i26; idx++) {
//				anStringArray4[idx] = "Teleport";
//			}
//		}
		if (componentOptionsArray != null) {
			for (int i = 0; i < componentOptionsArray.length; i++) {
				if (componentOptionsArray[i].equalsIgnoreCase("Delete All")) {
					if (i - 1 >= 0 && componentOptionsArray[i - 1].equalsIgnoreCase("delete")) {
						componentOptionsArray[i - 1] = "Next";
						componentOptionsArray[i] = "Quests";
					}
					if (componentOptionsArray[i].equals("Delete")) {
						componentOptionsArray[i] = "Next Page";
					} else if (componentOptionsArray[i].equals("Delete all")) {
						componentOptionsArray[i] = null;
					}
				}
			}
		}
		...
		//System.out.println("4784: " + aString4784);
		...
		//System.out.println("4786: " + aString4786);
		int i_32_ = -1;
	...
	final void method4143(String string, int i, int i_41_) {
		if (componentOptionsArray == null || i >= componentOptionsArray.length) {
			String[] strings = new String[i + 1];
			if (componentOptionsArray != null) {
				for (int i_42_ = 0; (i_42_ ^ 0xffffffff) > (componentOptionsArray.length ^ 0xffffffff); i_42_++) {
					strings[i_42_] = componentOptionsArray[i_42_];
				}
			}
			componentOptionsArray = strings;
		}
		if (i_41_ != 0) {
		...
		anInt4731++;
		int i_42_ = this.parentId >> 16;
		if (i_42_ == 17 * 2 && (string.equals("Edit") || string.equals("Colour") || string.equals("Delete"))) {
			componentOptionsArray[i] = null;
		} else {
			componentOptionsArray[i] = string;
		}
	}
