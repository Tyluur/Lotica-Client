...
public class Node_Sub34 extends Node {
	...
	static final String method2742(IComponentDefinitions widget, int i, int i_3_) {
		...
		if (widget.componentOptionsArray == null || (i ^ 0xffffffff) <= (widget.componentOptionsArray.length ^ 0xffffffff) || widget.componentOptionsArray[i] == null || (widget.componentOptionsArray[i].trim().length() ^ 0xffffffff) == -1) {
			if (Class54.aBoolean817) {
				return "Hidden-" + i;
			}
			return null;
		}
		return widget.componentOptionsArray[i];
	}
