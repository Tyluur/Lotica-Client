...
public class SignLink implements Runnable {
	...
	private final Class241 method3643(int i, int i_11_, Object ipAddress, int i_12_, int port) {
		Class241 class241 = new Class241();
		class241.ipAddress = ipAddress;
		class241.port = port;
		if (i_12_ != 9) {
			aClass241_3983 = null;
		}
...
//		System.out.println("ip=" + ipAddress + ",port=" + port + "\t" + (ipAddress == null ? "" : ipAddress.getClass()));
		return class241;
	}
		...
		for (; ; ) {
			...
			try {
				...
				if ((i ^ 0xffffffff) == -2) {
					...
					class241.anObject2956 = new Socket(InetAddress.getByName((String) class241.ipAddress), class241.port);
//					System.err.println("new socket constructed @ " + class241.anObject2956);
				} else if (i != 22) {
					if ((i ^ 0xffffffff) != -3) {
						if (i != 4) {
							if (i != 8) {
								if ((i ^ 0xffffffff) == -10) {
									Object[] objects = (Object[]) class241.ipAddress;
									...
									Transferable transferable = (Transferable) class241.ipAddress;
									Clipboard clipboard = Toolkit.getDefaultToolkit().getSystemClipboard();
									...
									if ((i ^ 0xffffffff) == -4) {
										...
										String string = String.valueOf(class241.port >> 24 & 0xff) + "." + ((0xffd9e4 & class241.port) >> 16) + "." + (class241.port >> 8 & 0xff) + "." + (0xff & class241.port);
										class241.anObject2956 = InetAddress.getByName(string).getHostName();
									} else if (i == 21) {
										if (Class313.method3650(false) < aLong3999) {
											throw new IOException();
										}
										class241.anObject2956 = InetAddress.getByName((String) class241.ipAddress).getAddress();
										...
										if ((i ^ 0xffffffff) == -7) {
											...
											if (aBoolean3985) {
												aClass11_3997.method191(frame, class241.anInt2951 >> 16, 0xffff & class241.anInt2951, 0xffff & class241.port, 106, class241.port >>> 16);
											} else {
												Class.forName("Display").getMethod("enter", new Class[]{aClass4006 == null ? aClass4006 = method3646("java.awt.Frame") : aClass4006, Integer.TYPE, Integer.TYPE, Integer.TYPE, Integer.TYPE}).invoke(anObject3980, new Object[]{frame, new Integer(class241.port >>> 16), new Integer(0xffff & class241.port), new Integer(class241.anInt2951 >> 16), new Integer(0xffff & class241.anInt2951)});
											}
										} else if (i == 7) {
											if (aBoolean3985) {
												aClass11_3997.method192((Frame) class241.ipAddress, 8);
											} else {
												Class.forName("Display").getMethod("exit", new Class[0]).invoke(anObject3980, new Object[0]);
											}
										} else if ((i ^ 0xffffffff) == -13) {
											FileOnDisk fileondisk = method3630(aString3987, anInt3978, (String) class241.ipAddress, 19613);
											class241.anObject2956 = fileondisk;
										} else if (i != 13) {
											if (aBoolean4005 && (i ^ 0xffffffff) == -15) {
												int i_16_ = class241.port;
												...
												boolean bool = (class241.port ^ 0xffffffff) != -1;
												Component component = (Component) class241.ipAddress;
												if (aBoolean3985) {
												...
												Object[] objects = (Object[]) class241.ipAddress;
												Class.forName("Class208").getDeclaredMethod("method2047", new Class[]{aClass4007 == null ? aClass4007 = method3646("java.awt.Component") : aClass4007, aClass4008 == null ? aClass4008 = method3646("[I") : aClass4008, Integer.TYPE, Integer.TYPE, aClass4009 == null ? aClass4009 = method3646("java.awt.Point") : aClass4009}).invoke(anObject3990, new Object[]{(Component) objects[0], (int[]) objects[1], new Integer(class241.port), new Integer(class241.anInt2951), (Point) objects[2]});
											} else if (i == 16) {
												try {
													if (!aString3981.startsWith("win")) {
														throw new Exception();
													}
													String string = (String) class241.ipAddress;
											...
											FileOnDisk fileondisk = method3630("", anInt3978, (String) class241.ipAddress, 19613);
											class241.anObject2956 = fileondisk;
										}
							...
							class241.anObject2956 = new DataInputStream(((URL) class241.ipAddress).openStream());
						}
					} else {
						Thread thread = new Thread((Runnable) class241.ipAddress);
						thread.setDaemon(true);
						thread.start();
						thread.setPriority(class241.port);
					...
					try {
						class241.anObject2956 = Node_Sub38_Sub14.method2834((byte) -117, (String) class241.ipAddress, class241.port).method435(30749);
	...
	private static final FileOnDisk method3630(String string, int i, String string_5_, int i_6_) {
		...
		String[] strings = {ClientConstants.CACHE_LOCATION};
	...
	final Class241 method3629(String ipAddress, boolean bool, boolean bool_4_, int port) {
		if (!bool) {
			return null;
		}
		return method3643(0, bool_4_ ? 22 : 1, ipAddress, 9, port);
	}
