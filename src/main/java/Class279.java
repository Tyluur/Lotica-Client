...
public class Class279 {
	...
	final NpcDefinition getNPCDefinitions(int i, byte b) {
		...
		npcdefinition.id = i;
		npcdefinition.rightClickOptions = (String[]) aStringArray3568.clone();
		if (bs != null) {
			npcdefinition.method3000((byte) 61, new BufferedStream(bs));
		}
		npcdefinition.method2997((byte) -110);
		applyCustomDefinitions(npcdefinition);
		synchronized (aClass61_3560) {
	...
	private void applyCustomDefinitions(NpcDefinition definition) {
		switch (definition.id) {
			case 1263:
				definition.rightClickOptions = new String[]{"Talk-to", null, "Previous", null, null, "Examine"};
				break;
			case 1597:
				definition.rightClickOptions = new String[]{"Talk-to", null, "Get-task", "Trade", null, "Examine"};
				break;
		}
//		System.out.println(definition.id + "\t[" + Arrays.toString(definition.rightClickOptions) + "]");
	}
