...
public class FixedAnimator extends Animator {
	...
	static final void method258(boolean bool, byte b, boolean bool_0_, int i, int i_1_, String string, String string_2_) {
		...
		for (int i_6_ = 0; EntityNode_Sub3_Sub1.aClass86_9166.anInt1175 > i_6_; i_6_++) {
			...
			if ((!bool_0_ || itemdefinition.showInGrandExchange) && itemdefinition.certTemplateId == -1 && itemdefinition.lendTemplateId == -1 && (itemdefinition.unknownValue1 ^ 0xffffffff) == 0 && itemdefinition.dummyItem == 0 && itemdefinition.name.toLowerCase().indexOf(string) != -1) {
				if (!itemdefinition.isExchangeable()) {
					continue;
				}
				if ((i_1_ ^ 0xffffffff) != 0) {
					if (!bool) {
